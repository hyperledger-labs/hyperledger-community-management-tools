...
git log --invert-grep --grep='Signed-off-by' --pretty='%aN <%aE>' ${since:+--since=${since}} ${until:+--until=${until}}| git -c mailmap.file=${script_dir}/mailmap check-mailmap --stdin > "${outdir}"/working/${repo}.gitlog
...
awk 'BEGIN { FS = "|" }
     {
       ...
       gsub(/,/,"|",col2)
       print col1 "," col2
     }' "${outdir}"/working/contributors > "${outdir}"/signed_off_by_audit.csv
